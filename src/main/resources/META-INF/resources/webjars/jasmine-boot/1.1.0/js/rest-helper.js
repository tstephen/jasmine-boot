  ...
  me.ajax = function(options) {
    try {
      ...
      xhr.onload = function() {
        ...
        try {
          if (options['success']!=undefined && typeof options['success'] == 'function') {
            switch (true) {
            case (xhr.responseText == null || xhr.responseText == ''):
              options['success']('', xhr.status, xhr);
              break
            case (xhr.responseXML != null):
              options['success'](xhr.responseXML, xhr.status, xhr);
              break;
            default:
              try {
                options['success'](JSON.parse(xhr.responseText), xhr.status, xhr);
              } catch (e) {
                console.log('Unable to parse JSON from response, send text: '+xhr.responseText);
      ...
      if (options['data']==undefined) {
        console.log('  sending with no data');
        xhr.send();
      } else {
  ...
  me.uri = function(entity, entityPath) {
    ...
    try {
      if (entity['links']!=undefined) {
        ...
        console.error('Entity at path '+entityPath+' is not HAL compliant: '
